...
void Population::expandPop(){
    ...
    for(int i = popSize; i<2*popSize-1; i++){
        ...
        child.setFitness(std::numeric_limits<double>::min());
...
void Population::injectNewIndividual(){
    ...
    newRobot.setFitness(std::numeric_limits<double>::min());
...
void Population::writeGenerationStatsForRun(int genNum, int runNum){
    std::ofstream myfile;
    std::string name = "/Users/JeanBaptisteBolh/Documents/School/CS352/Final/GA_Distance/Outputs/Run_" + std::to_string(runNum) + ".txt";
...
void Population::writeWholePopulationToFile(int genNum){
    std::ofstream myfile;
    std::string name = "/Users/JeanBaptisteBolh/Documents/School/CS352/Final/GA_Distance/Outputs/Gen_" + std::to_string(genNum) + ".txt";
    myfile.open (name, std::ios::app);
    for(int i=0; i<popSize; i++){
...
void Population::setIndivCPG(int i, std::vector<double> params){
    individuals[i].setCPGParameters(params);
}
void Population::setIndivMorph(int i, std::vector<double> params){
    individuals[i].setMorphParameters(params);
}
void Population::setIndivParams(int i, std::vector<double> params){
    individuals[i].setParameters(params);
}
